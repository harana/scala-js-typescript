...
class ContainerSymbol(nme: Name) extends Symbol(nme) {
  ...
  def newField(name: Name, modifiers: Modifiers, optional: Boolean = false): FieldSymbol = {
    val result = new FieldSymbol(name, modifiers, optional)
    members += result
    result
  }
...
class FieldSymbol(nme: Name, val modifiers: Modifiers, val optional: Boolean = false) extends Symbol(nme) with JSNameable {
  var tpe: TypeRef = TypeRef.Any

  override def toString() = s"${jsNameStr}${if (modifiers(Modifier.ReadOnly)) "val" else "var"} $name: $tpe - $optional"
}
