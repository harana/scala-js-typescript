...
class TSDefParser extends StdTokenParsers with ImplicitConversions {
  ...
  lexical.reserved ++= List(
      ...
      "keyof", "new", "return", "switch", "this", "throw", "try", "typeof",
      "var", "void", "while", "with",
  ...
  lazy val ambientDeclaration: Parser[Option[DeclTree]] = ((
      opt("declare") ~> opt("export") ~> opt("import") ~> moduleElementDecl1
    | opt("export") ~> opt("default") ~> opt("declare") ~> moduleElementDecl1
  ).map(Some(_))
    | "export" ~> opt("import") ~> lexical.Identifier("as") ~> "namespace" ~> identifier <~ opt(";") ^^^ None
  )
  ...
  lazy val topLevelExportDecl: Parser[DeclTree] =
    opt("=") ~> identifier <~ ";" ^^ TopLevelExportDecl

  lazy val moduleElementDecl: Parser[Option[DeclTree]] = (
      "export" ~> opt("import") ~> opt("default") ~> (
